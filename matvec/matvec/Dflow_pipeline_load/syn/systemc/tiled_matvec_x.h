...
#ifndef _tiled_matvec_x_H_
#define _tiled_matvec_x_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "tiled_matvec_x_memcore.h"

SC_MODULE(tiled_matvec_x) {
...
protected:
    ...
    tiled_matvec_x_memcore* tiled_matvec_x_memcore_U;
...
public:
    ~tiled_matvec_x();
    SC_CTOR(tiled_matvec_x) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        tiled_matvec_x_memcore_U = new tiled_matvec_x_memcore("tiled_matvec_x_memcore_U");
        tiled_matvec_x_memcore_U->clk      ( clk );
        tiled_matvec_x_memcore_U->reset    ( reset );
        tiled_matvec_x_memcore_U->ce0      ( i_ce0 );
        tiled_matvec_x_memcore_U->we0      ( i_we0 );
        tiled_matvec_x_memcore_U->address0 ( memcore_iaddr );
        tiled_matvec_x_memcore_U->d0       ( i_d0 );
        tiled_matvec_x_memcore_U->q0       ( i_q0 );

        tiled_matvec_x_memcore_U->ce1      ( t_ce0 );
        tiled_matvec_x_memcore_U->we1      ( t_we0 );
        tiled_matvec_x_memcore_U->address1 ( memcore_taddr );
        tiled_matvec_x_memcore_U->d1       ( t_d0 );
        tiled_matvec_x_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "tiled_matvec_x.trace");
            // ports
