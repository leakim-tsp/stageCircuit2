...
#ifndef _tiled_matvec_A_H_
#define _tiled_matvec_A_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "tiled_matvec_A_memcore.h"

SC_MODULE(tiled_matvec_A) {
...
protected:
    ...
    // buffer instances
    tiled_matvec_A_memcore* buffer[BufferCount];
...
public:
    ~tiled_matvec_A();
    SC_CTOR(tiled_matvec_A) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "tiled_matvec_A_memcore_U_";
            name += szTmp;

            buffer[i] = new tiled_matvec_A_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "tiled_matvec_A.trace");
            // ports
